...
- 查询轮换覆盖软件、云、安全和计算机服务；计算机服务只增加明确的“市场经理”关键词，不进行自动词汇扩展或自学习。每轮先查看全部查询的卡片，再按非空候选池公平分配完整 JD 额度；空结果或候选不足的额度可借给其他查询。最多一次补充分配，仍不超过 8 份完整 JD。
- 轮换中另有一条“市场 / 软件 / 市场营销策划”的原生职位类型筛选（`position=140101`），不强加给所有方向。位置参数只接受已确认的平台代码，并与查询词、城市、行业一起进行精确页面核对；缺失或错误类型不能通过读取。PBD 等其他商业方向仍保留宽口径查询，这不是自动学习或任意新增平台参数。
- 同一来源 ID 在一轮内只读取一次完整 JD。池内优先可见卡片信息有变化或尚未精读的岗位，阅读顺序参考已配置的职责方向，不以标题决定是否匹配。私有 `read-history.json` 仅保存最近 2000 个完整读取的时间与卡片指纹，不保存额外原文；至少 7 天未读或旧台账无时间的记录保留一个有界复查机会。最近读过的记录在额度剩余时仍可复查，不按待复核/不入选结论永久屏蔽。JD 内容可能在卡片未变时更新，所以不会把已知 ID 当作永远不变。
- 只通过 Chrome 已授权的 Apple Events JavaScript 读取公开搜索卡片和匹配的职位详情。不会读取 Cookie、简历、消息或账号面板，也不会沟通、收藏、投递或修改账户。不会绕过验证码或启用自动化伪装。
- 验证码、登录失效、网络或页面结构异常均显式阻塞或失败，不当作“没有岗位”；未经精确 ID/标题核对的详情不能作为有效 JD。自动时段失败后不在每分钟重复重试；下一时段才再次尝试。
- 如果同一公开 ID 的详情标题连续多次与卡片标题不同，该记录单独进入私有复核，不读取或接纳其 JD、不计入完整 JD 数，也不因此关闭旧岗位。继续处理其他通过精确 ID/标题核对的记录；若没有任何可完整读取的候选则本轮阻塞。错误 ID、未知加载、登录或验证码仍不能当作空结果或成功读取。
- ID 与标题匹配但连续多次仅有非空的过短描述时，记录 `jd-content-incomplete` 到私有复核，并将完整 JD 配额借给其他候选；短描述不计作完整 JD，也不被规则自动选中。空白或“加载中”仍须等待/阻塞，不能靠此分支假装读取成功。
- 职责与任职要求一起决定规则初筛。未能确认的硬性要求进入本机私有复核记录。新规则初筛不代表满足全部资格；既有人工辅助评估不会被自动覆盖。原 JD 只保存在私有运行目录，公开摘要使用通用改写，不公开个人匹配配置。
- 段落解析先规范 Unicode 展示形式，再识别明确职责/任职要求/加分条件；清晰的编号职责块可位于任职要求标题之前，但资格条件仍单独评估。真正缺少分区或语义含糊的文本留待复核，解析失败不是已经不匹配。公开快照可只展示通用待复核数量和其中的结构解析数量，不公开队列中的岗位、私人判断或原文，也不把没有入选新增解读为市场没有机会。
...
明确的人工撤回记录保存在私有 `manual-exclusions.json`（`0600`），与累计证据台账分开。记录按来源限定的岗位 ID 排除，包含明确的用户请求原因码和记录时间，不推导新的职业偏好或类别规则。重新启用服务后，采样候选及最终合并都会遵守这些排除；撤回不删除私有历史证据。无效或权限不安全的排除文件会阻塞运行，不能被忽略。

本机运行摘要也列出各查询的卡片、实际完整 JD、首次精读和复查数量；计数来自真实读取，不是分配了额度就视作读取成功。任务整体成功仅表示本轮有界采样与发布完成，不表示已经人工清空待复核队列。

修复上一个已失败或阻塞时段后，可显式执行 `retry-slot`。它只重试该时段确认过的查询轮换，以独立重试 ID 保留原失败记录，不清空已尝试时段、不自动循环重试，也不推进到下一组查询。已有待确认推送时改用下面的发布恢复流程。

手动验收运行遇到同样问题时，使用 `retry-run` 明确重试上一次失败运行的原查询轮换，而不是切换到另一组查询掩盖失败。
可以使用 `retry-run --dry-run` 明确进行同轮换的非发布复查；它不会推送，也不会消耗尚未尝试的自动时段。

## 状态、恢复和发布

单轮使用系统 `flock` 排他锁，由 macOS 自带 Perl 的标准 `Fcntl` 模块持有；主进程退出导致管道关闭时由内核释放。锁拥有者 JSON 先在私有临时文件完整写入，再通过排他的原子硬链接发布，不会覆盖同时到达的旧版锁，也不存在先创建空锁再补写的窗口。旧版不完整锁/恢复标记仅在足够陈旧、未发现活跃拥有者和打开该文件的进程后清理，使用系统 `lsof` 检查；未写完的 PID 数字前缀不能当成活跃进程号，不会仅因文件陈旧就删除活跃锁。失败、阻塞、取消与成功分别持久化。自动时段在开始前记为已尝试，进程重启不会把失败时段当作尚未开始而无限循环。日志只记录通用错误码和计数，单文件有界轮转；保留最近 30 轮私有证据。
