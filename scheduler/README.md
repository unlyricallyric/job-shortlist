...
初次安装是暂停状态。可以用 `--adopt-window <ID> --adopt-tab <ID>` 接管**明确属于此任务**的 BOSS 搜索标签，不要传入用户其他标签。未指定时，程序会寻找 BOSS 标签以创建自己的后台搜索标签，不激活 Chrome。标签句柄只保存在私有状态中，每次脚本执行都核对公开来源和路径。用户关闭任务标签或重启 Chrome 后，只有确认旧窗口/标签已经不存在，才会借助已重新打开的 BOSS 标签所在窗口创建**新的任务标签**；不会复用该用户标签。若用户把仍存在的任务标签导航到其他页面，程序会阻塞，需明确重新指定任务标签，不能绕过权限、登录或验证状态。
...
修复上一个已失败或阻塞时段后，可显式执行 `retry-slot`。它只重试该时段确认过的查询轮换，以独立重试 ID 保留原失败记录，不清空已尝试时段、不自动循环重试，也不推进到下一组查询。已有待确认推送时改用下面的发布恢复流程。

## 状态、恢复和发布

单轮使用系统 `flock` 排他锁，由 macOS 自带 Perl 的标准 `Fcntl` 模块持有；主进程退出导致管道关闭时由内核释放。锁拥有者 JSON 以临时文件原子替换，不存在先创建空锁再补写的窗口。旧版不完整锁/恢复标记仅在足够陈旧、未发现活跃拥有者和打开该文件的进程后清理，使用系统 `lsof` 检查；不会仅因文件陈旧就删除活跃锁。失败、阻塞、取消与成功分别持久化。自动时段在开始前记为已尝试，进程重启不会把失败时段当作尚未开始而无限循环。日志只记录通用错误码和计数，单文件有界轮转；保留最近 30 轮私有证据。
