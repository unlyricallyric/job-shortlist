...
已确认绑定的任务标签若切换到了同源的精确 `/web/geek/chat` 沟通路由，会**保留原页、不读取其 DOM/聊天/联系人**，在同一已确认的普通窗口另建最多一个搜索标签，并把新的精确句柄作为任务绑定。登录通常属于 Chrome profile 的站点会话，不需要依靠每个标签单独登录，但新页仍实际检查登录、验证码与结果可读性，不能假定会话永不过期。创建前再次确认原标签未变化；用户并发切换页面/窗口时停止，不覆盖或关闭任何原页。后续运行复用新的搜索页，不再新建。日志仅记录通用恢复原因，不包含沟通 URL 查询参数或页面内容。

私人 `browser-context.json` 记录普通 Chrome 主进程身份和恢复阶段，不保存登录数据。单实例、同用户、正常窗口及已知绑定用于确定原上下文；多实例、自动化启动参数、隐身窗口、Chrome 重启后无法确认原绑定、原窗口和任务标签均消失时，明确阻挡，不随便选另一个可能属于不同 profile 的窗口。上述已知沟通路由以外的非搜索页保持原样并阻挡；认证/验证码路由不会通过另开页绕过，未验证的新任务页也不会递归创建第二个页。
...
| `browser-preserved-page-changed` | 沟通页在恢复前发生变化，原页保持不动 |
| `unexpected-owned-tab` | 非搜索且不是可安全保留的已知沟通路由，或新任务页尚非搜索页；保持原样 |
