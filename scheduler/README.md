...
私人 `browser-context.json` 记录普通 Chrome 主进程身份和恢复阶段，不保存登录数据。单实例、同用户、正常窗口及已知绑定用于确定原上下文；多实例、自动化启动参数、隐身窗口、Chrome 重启后无法确认原绑定、原窗口和任务标签均消失时，明确阻挡，不随便选另一个可能属于不同 profile 的窗口。现有任务页被用户导航到别处也不会被替换。
...
| `unexpected-owned-tab` | 任务页已被导航到非允许页面，保持原样 |
