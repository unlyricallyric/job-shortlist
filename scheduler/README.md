...
项目架构见 [ARCHITECTURE.md](../ARCHITECTURE.md)。换机使用
[Mac-to-Mac 迁移指南](MIGRATION.md)中的 `migration.mjs`，不要复制整个运行目录、
浏览器或凭据。工具在仓库内运行，不要求重装现有服务；只允许一台电脑拥有活跃调度。
...
方向政策 `partner-commercial-v1` 的旧六项仍可验证；`partner-commercial-v2` 支持上述八项中明确配置的、无重复的有界轮换。默认八项中，从游标 0 开始的前三轮依次取 0–2、3–5、6–7 加 0；追加时保留游标，不伪造已执行记录或把新词强行放到首轮。安装保留已有 `intent-policy.json` 中合法的自定义子集/顺序，并让运行查询与该文件一致，不用新默认值覆盖用户配置。增加轮换词不表示已经实采或保证相关性。
...
`role-feedback-v3` 另增可显式启用的 `outsourced-employment` 用工安排排除。标题明确标注外包/劳务派遣，或来源明确说明本岗位由第三方外包签约并派驻客户方时，才按此类别处理；卡片标题已有明确安排时不需借用不完整或身份冲突的 JD。非外包/非派遣、正式直签、管理外包团队、销售外包服务、客户的外包伙伴、招聘代理介绍均不自动等同于本人的外包雇佣，孤立驻场/合同/第三方或公司名称也不能据此判断。未知安排仍待确认，不推断正式雇佣。v1/v2 历史原样保留，新版只追加相应类别与原证据观察时间；导出恢复同时校验声明版本与历史。

`role-feedback-v4` 增加 `sales-leadership` 与 `automotive-project`：明确的一般销售总监/销售团队管理、本人整车研发项目/交付及强制整车经验标题可以直接提供负向证据，不因 JD 未读、缺失或身份冲突变成“无从判断”。科技公司或 IDC/GPU/云等产品词不是职责豁免。真实已验证的渠道伙伴经营职责、明确伙伴/渠道总监、伙伴营销赋能，以及客户处于汽车行业的科技合作业务不会一概排除；错误或旧版 JD 不能覆盖标题。v1–v3 历史版本、依据与时间原样保留，新判断按 v4 追加，所有发布/恢复路径与迁移校验使用声明版本。
...
每个当前 ID 必须且只能出现一次，不能只提交部分移除名单；`decision` 为 `remove`、`retain` 或 `uncertain`，后两者的 `category` 为 `null`。移除类别须属于当前已声明的扩展政策；快照字节变化时整次操作失败，不猜测增删记录。实际文件必须覆盖所有已选入和候选记录，上例仅示意一个字段结构。
...
所有采集/人工队列修改/发布使用同一内核 `flock`，拥有者声明采用不可覆盖的原子发布。程序退出会释放管道及锁；超时/取消清理仅限自己创建的子进程组。

### 普通 Chrome 任务页恢复

`boss-tab-required` 是**任务页上下文未建立**，不是 Cookie、登录过期或验证码的检测结果。旧版在保存的窗口/标签 ID 丢失后，还要求另一个已打开的 BOSS 页面作“种子”；关闭所有 BOSS 标签后，即使 Chrome 登录仍有效也会报这个错误。

恢复不再依赖额外种子页：当保存的普通窗口仍存在且任务标签确实缺失时，在原窗口最多创建一个专用标签，保持其他标签和当前焦点，再验证实际搜索页面的登录/验证码/可读状态。现有合法任务页不重复创建；同一任务标签移到另一个窗口时按精确标签 ID 重新绑定，不读其他页面内容。首次安装尚无任务绑定时，仍需用户打开明确的源站页面来建立上下文。

已确认绑定的任务标签若切换到了精确 `https://www.zhipin.com` 同源的普通页面（包括主页、沟通及其他站内页面），会**保留原页、不读取其 DOM/聊天/联系人**，在同一已确认的普通窗口另建最多一个搜索标签，并把新的精确句柄作为任务绑定。登录通常属于 Chrome profile 的站点会话，不需要依靠每个标签单独登录，但新页仍实际检查登录、验证码与结果可读性，不能假定会话永不过期。创建前与创建时核对原窗口/标签归属及同源和认证边界，不要求普通站内路径或查询参数始终相同。用户正常操作其他标签或焦点不阻止恢复，也不会被强制恢复到旧焦点；不覆盖或关闭原页。后续运行复用新的搜索页，不再新建。日志仅记录通用恢复原因，不包含页面 URL 查询参数或内容。

私人 `browser-context.json` 记录普通 Chrome 主进程身份和恢复阶段，不保存登录数据。单实例、同用户、正常窗口及已知绑定用于确定原上下文；多实例、自动化启动参数、隐身窗口、Chrome 重启后无法确认原绑定、原窗口和任务标签均消失时，明确阻挡，不随便选另一个可能属于不同 profile 的窗口。跨来源、非 HTTPS、不同端口或带 URL 凭据的页面保持原样并阻挡；已识别的认证/验证码路由不会通过另开页绕过，未验证的新任务页也不会递归创建第二个页。

窗口/GUI 暂不可访问时最多约 12 秒有界复查；权限拒绝和未知访问错误不会被当作“标签不存在”。新标签初始空白不算成功，实际源站可读性按既有最多 45 秒等待与登录/验证码保护校验。若认证或读取失败，保留自己创建的精确任务句柄，后续只复查同一页；创建命令结果不确定时保存未确认阶段并停止，避免再次创建而泄漏标签。不要为了重试而直接删除未确认恢复记录；应先人工核对本次任务页。

| 错误 | 含义 |
| --- | --- |
| `boss-tab-required` | 初始任务上下文尚未建立，未检测登录 |
| `chrome-not-running` | 普通 Chrome 未运行；程序不会启动实例 |
| `chrome-gui-unavailable` | 在有界等待内 GUI/窗口仍不可用或未响应 |
| `browser-owner-window-missing` | 原窗口和任务标签均找不到，不能确认其他窗口的 profile |
| `browser-context-changed` / `browser-context-ambiguous` | 原进程发生变化或普通实例上下文不唯一，需明确重新确认 |
| `browser-recovery-unconfirmed` | 之前创建结果不确定，禁止再盲建标签 |
| `browser-preserved-page-changed` | 原窗口/标签归属或同源边界在创建前发生变化，原页保持不动 |
| `unexpected-owned-tab` | 既有页不在可确认的同源范围，或新任务页尚非搜索页；保持原样 |
| `apple-events-denied` / `apple-events-javascript-disabled` | 实际系统/Chrome 授权阻挡 |
| `login-required` / `captcha` | 来源页面实际显示认证要求，需用户处理 |

睡眠/DarkWake 可能让 GUI 不可用，但同一错误也可能由用户关闭页面或浏览器上下文变化触发。时段 ID、真实开始/结束和恢复等待日志各自保留，不能据一次错误直接断言睡眠或登录失效是根因。本修复不唤醒电脑、不阻止用户睡眠、不改变原日程，也不承诺在真正系统睡眠中运行；超时后不会自行追加采集轮次。
